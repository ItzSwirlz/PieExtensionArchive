          ...
          constructor(twitchBlobUrl, options) {
              ...
              this.onmessage = function(e) {
                  if (e.data.key == 'ShowAdBlockBanner') {
                      ...
                      if (timer === 0) {
                        timer = 1;
                        adBlockDiv.insertAdjacentHTML('beforeend', '<div id="adBlocked" style="display: none;"></div>');
                        interval = setInterval(() => {
                            timer++;
                            const minutes = Math.floor(timer / 60);
                            const seconds = timer % 60;
                            const formattedTime = `${minutes}:${seconds.toString().padStart(2, '0')}`;
                            adBlockDiv.P.textContent = `Blocking ads: ${formattedTime} saved`;
                            if (timer === 180) {
                              clearInterval(interval); // clear timer after 3 mins to prevent it from going forever
                              adBlockDiv.P.textContent = '';
                            }
                        }, 1000);
                      }
                      if (adBlockDiv.P.textContent.includes('Blocking ads') == false) {
                        adBlockDiv.P.textContent = 'Blocking ads';
                      }
                      if (document.querySelector('.video-player').classList.contains('gradient-border') == false) {
                          document.querySelector('.video-player').className += ' gradient-border';
                      }
                      ...
                      document.querySelector('.video-player').classList.remove('gradient-border');
                      let infoDiv = document.querySelector('#infoDiv');
                      if (!infoDiv) {
                        adBlockDiv.insertAdjacentHTML('beforeend', '<div id="infoDiv" style="display: none;"></div>');
                        infoDiv = document.querySelector('#infoDiv');
                      }
                      infoDiv.setAttribute('data-id', Date.now());
                      infoDiv.setAttribute('data-ad-time', timer);
              ...
              function getAdBlockDiv() {
                  ...
                  if (playerRootDiv != null) {
                      ...
                      if (adBlockDiv == null) {
                          ...
                          const GRADIENT_CSS = 'linear-gradient(125deg, #AD4ABB 23.57%, #696AF6 59.28%, #38B4A8 92.99%);'
                          adBlockDiv.innerHTML = `<div class="player-adblock-notice" style="display: flex; color: white; border-radius:8px; background: #292929; position: absolute; top: 0px; right: 0px; padding: 5px;">
                           <img src="https://cdn.pie.org/pie-adblock-logo-blue.png" style="width: 20px; margin-right: 8px;" /> <p></p>
                          </div>`;
                          adBlockDiv.style.display = 'none';
                          ...
                          document.head.insertAdjacentHTML('beforeend', `
                            <style id="pstyles">
                              .gradient-border {
                                border: 2px solid #AD4ABB; /* Initial border color */
                                border-radius: 3px;
                                animation: animatedgradient 2s infinite alternate; /* Animate over 2 seconds */
                              }
                              @keyframes animatedgradient {
                                0% {
                                  border-color: #AD4ABB;
                                //   border-image: linear-gradient(125deg, #AD4ABB 23.57%, #696AF6 59.28%, #38B4A8 92.99%);
                                }
                                50% {
                                  border-color: #696AF6;
                                //   border-image: linear-gradient(125deg, #AD4ABB 23.57%, #696AF6 59.28%, #38B4A8 92.99%);
                                ...
                                100% {
                                 border-color: #38B4A8;
                                //  border-image: linear-gradient(125deg, #AD4ABB 59.28%, #696AF6 92.99%, #38B4A8 23.57%);
                                }
                              }
                            </style>
                          `);
